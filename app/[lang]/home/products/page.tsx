...
function normalizeProductImages(data?: ProductImagesResponse): ProductImageItem[] {
...
function resolveLastId(data: ProductsResponse | undefined, fallbackItems: ProductItem[]): number | null {
...
function buildProductsQuery(dil: number, pageSize: number, search: string, lastId?: number | null) {
...
export default function ProductsHome() {
  ...
  const imageCacheRef = useRef<Record<string, ProductImageItem[]>>({});
  const imageRequestRef = useRef<Partial<Record<string, Promise<ProductImageItem[]>>>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const qs = buildProductsQuery(dil, 4, search);
        const data = await api.get<ProductsResponse>(`/api/products?${qs.toString()}`);
        if (cancelled) return;
        ...
        if (cancelled) return;
        setError(String(err?.message ?? "Failed to load products"));
        setItems([]);
        setLastId(null);
      } finally {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const fetchNextPageOrRestart = async () => {
      ...
      try {
        ...
        const qs = buildProductsQuery(dil, 4, search, lastId);
        const data = await api.get<ProductsResponse>(`/api/products?${qs.toString()}`);
        if (cancelled) return;
        ...
        // End of cursor pagination: restart from first page.
        const restartQs = buildProductsQuery(dil, 4, search);
        const restartData = await api.get<ProductsResponse>(`/api/products?${restartQs.toString()}`);
        if (cancelled) return;
        ...
        if (cancelled) return;
        setError(String(err?.message ?? "Failed to rotate products"));
        setTransitionLoading(false);
      } finally {
  ...
  useEffect(() => {
    ...
    const qs = new URLSearchParams({
      urunNr: String(currentNr),
      kaynak: "2",
      dil: String(dil),
    });

    void api.post(`/api/products/view?${qs.toString()}`).catch(() => undefined);
  }, [dil, item?.Nr]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!request) {
        const qs = new URLSearchParams({ urunNr: String(currentNr), dil: String(dil) });
        request = api
          .get<ProductImagesResponse>(`/api/products/images?${qs.toString()}`)
          .then((data) => {
            const nextImages = normalizeProductImages(data);
  ...
  useEffect(() => {
    ...
    for (const product of items) {
      const cacheKey = `${dil}-${product.Nr}`;
      if (imageCacheRef.current[cacheKey] || imageRequestRef.current[cacheKey]) continue;

      const qs = new URLSearchParams({ urunNr: String(product.Nr), dil: String(dil) });
      const request = api
        .get<ProductImagesResponse>(`/api/products/images?${qs.toString()}`)
        .then((data) => {
          const nextImages = normalizeProductImages(data);
  ...
  const goToImageIndex = (nextIndex: number) => {
    if (items.length === 0) return;
  ...
  const handleLikeClick = () => {
    ...
    void api.post(`/api/products/favorite?${qs.toString()}`).catch(() => undefined);
  };
  ...
  const title = item?.UrunAdi ?? t.home.productsTitle;
  const subtitle = item?.UrunEtiketList ?? "";
  const description = item?.UrunAciklamasi ?? "";
  const imageUrl = item?.VarsayilanResim || "/demo/watch.png";
  const favCount = item ? String(item.UrunFavcount ?? 0) : "0";
  const viewCount = item ? String(item.UrunBakcount ?? 0) : "0";
  const isCustomerFavorite = Boolean(item?.MusteriFavUrun);
  const loadingText = lang === "tr" ? "Yükleniyor..." : "Loading...";
  const emptyText = lang === "tr" ? "Ürün bulunamadı." : "No products found.";
  ...
  const tabs = [
    { label: t.home.productsTitle, href: `/${lang}/home/products`, active: true },
    { label: t.home.servicesTitle, href: `/${lang}/home/services` },
  ];
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-[1220px]">
          <div className="relative">
            <div className="grid gap-4 xl:grid-cols-[1.03fr_1fr] xl:gap-0">
              <div className="relative min-h-[560px] overflow-hidden rounded-[30px] bg-[#0f1116] xl:rounded-r-none">
                <div
                ...
                <div
                  className="relative flex h-full select-none items-start justify-center px-8 py-10"
                ...
                >
                  ...
                  <img
                    key={item?.Nr ?? "fallback-product-image"}
                    src={imageUrl}
                    alt={title}
                    draggable={false}
              ...
              <div className="relative min-h-[560px] rounded-[30px] border border-[#e5dcc0] bg-[#f2ebca] p-6 xl:rounded-l-none xl:p-8">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h1 className="text-2xl font-extrabold tracking-tight text-neutral-800 lg:text-4xl">{title}</h1>
                    <p className="mt-1 text-xl font-medium text-neutral-700 lg:text-3xl">{subtitle}</p>
                  </div>
                ...
                <p className="whitespace-pre-line text-sm leading-9 text-neutral-700 lg:text-[20px] lg:leading-[1.8]">
                  {descriptionText}
                </p>
                ...
                {item && productImages.length > 0 ? (
                  <div className="mt-6">
                    <div className="text-sm flex justify-end font-semibold text-neutral-700">
                      {lang === "tr" ? "Ürün görselleri" : "Product images"}
                    </div>
                      ...
                      {productImages.map((img) => (
                        <button
                          ...
                          className={`relative h-20 w-20 shrink-0 overflow-hidden rounded-lg border ${
                            img.UrunresimVarsayilan ? "border-amber-500" : "border-neutral-300"
                          }`}
                          title={img.UrunresimVarsayilan ? (lang === "tr" ? "Varsayılan resim" : "Default image") : undefined}
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <GtgLoading isLoading={transitionLoading || productImagesLoading} />
    </div>
  );
}
