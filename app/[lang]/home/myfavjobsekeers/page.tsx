...
type SeekerFeature = {
  ...
  MustozEhAcik?: string | null;
};

type FavoriteJobSeekerItem = {
  FavoriNr?: number;
  Nr?: number;
  MusteriNr?: number;
  ...
  FavoriAktif?: boolean | null;
  FavoriOlusturmaZamani?: string | null;
  Ozellikler?: SeekerFeature[] | null;
};
...
function itemLocation(item: FavoriteJobSeekerItem): string {
  const country = (item.UlkeAdi ?? "").trim();
  const city = (item.IlAdi ?? "").trim();
  return [country, city].filter(Boolean).join(" / ");
}

function featureLines(item: FavoriteJobSeekerItem): string[] {
  ...
  return list
    .map((feature) => {
      ...
      const extra = (feature.MustozEhAcik ?? "").trim();
...
export default function MyFavoriteJobSeekersPage() {
  ...
  const text = useMemo(() => {
    if (lang === "tr") {
      return {
        title: "Favori İş Arayanlarım",
        subtitle: "Favoriye aldığın iş arayanları buradan yönetebilirsin.",
        loading: "Favori iş arayanlar yükleniyor...",
        loadError: "Favori iş arayanlar yüklenemedi.",
        empty: "Favori iş arayan bulunamadı.",
        remove: "Favoriden Kaldır",
        removing: "Kaldırılıyor...",
        removeError: "Favori iş arayan kaldırılamadı.",
        search: "İş arayan ara",
        gender: "Cinsiyet",
        ...
        confirmBody: "Bu iş arayanı favorilerinden kaldırmak istediğine emin misin?",
    ...
    if (lang === "ru") {
      return {
        title: t.sidebar.items.favJobSeekers,
        subtitle: "Управляйте соискателями, добавленными в избранное.",
        loading: "Загрузка избранных соискателей...",
        loadError: "Не удалось загрузить избранных соискателей.",
        empty: "Избранные соискатели не найдены.",
        remove: "Удалить из избранного",
        removing: "Удаление...",
        removeError: "Не удалось удалить соискателя из избранного.",
        ...
        confirmBody: "Вы уверены, что хотите удалить этого соискателя из избранного?",
    ...
    if (lang === "es") {
      return {
        title: t.sidebar.items.favJobSeekers,
        subtitle: "Gestiona los candidatos que marcaste como favoritos.",
        loading: "Cargando candidatos favoritos...",
        loadError: "No se pudieron cargar los candidatos favoritos.",
        empty: "No se encontraron candidatos favoritos.",
        remove: "Quitar de favoritos",
        removing: "Quitando...",
        removeError: "No se pudo quitar el candidato de favoritos.",
        ...
        confirmBody: "¿Seguro que quieres quitar este candidato de favoritos?",
    ...
    if (lang === "fr") {
      return {
        title: t.sidebar.items.favJobSeekers,
        subtitle: "Gérez les candidats que vous avez ajoutés aux favoris.",
        loading: "Chargement des candidats favoris...",
        loadError: "Impossible de charger les candidats favoris.",
        empty: "Aucun candidat favori trouvé.",
        remove: "Retirer des favoris",
        removing: "Suppression...",
        removeError: "Impossible de retirer le candidat des favoris.",
        ...
        confirmBody: "Voulez-vous vraiment retirer ce candidat des favoris ?",
    ...
    return {
      title: t.sidebar.items.favJobSeekers,
      subtitle: "Manage the job seekers you have marked as favorite.",
      loading: "Loading favorite job seekers...",
      loadError: "Failed to load favorite job seekers.",
      empty: "No favorite job seekers found.",
      remove: "Remove from Favorites",
      removing: "Removing...",
      removeError: "Failed to remove favorite job seeker.",
      search: "Search job seekers",
      gender: "Gender",
      ...
      confirmBody: "Are you sure you want to remove this job seeker from favorites?",
      confirmApprove: "Yes, Remove",
      confirmCancel: "Cancel",
    };
  }, [lang, t.sidebar.items.favJobSeekers]);

  const [items, setItems] = useState<FavoriteJobSeekerItem[]>([]);
  ...
  const [confirmRemoveItem, setConfirmRemoveItem] = useState<FavoriteJobSeekerItem | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const response = await api.get<ApiResponse<FavoriteJobSeekerItem[]>>(`/api/jobseekers/favorites?${qs.toString()}`);
        if (cancelled) return;
  ...
  async function removeFavorite(item: FavoriteJobSeekerItem) {
    const musteriNr = toPositiveInt(item.MusteriNr ?? item.Nr);
    if (musteriNr == null || removeLoadingId != null) return;

    setRemoveLoadingId(musteriNr);
    try {
      await api.post(`/api/jobseekers/remove-favorite?favMusteriNr=${musteriNr}&kaynak=2&dil=${dil}`, {});
      setItems((prev) => prev.filter((it) => toPositiveInt(it.MusteriNr ?? it.Nr) !== musteriNr));
      ...
      <main className="px-4 py-6 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="flex min-h-[360px] flex-col items-center justify-center rounded-3xl bg-white text-center shadow-sm">
            <UserRound className="h-10 w-10 text-[#faa500]" />
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {items.map((item, index) => {
              const musteriNr = toPositiveInt(item.MusteriNr ?? item.Nr);
              const fullName = (item.AdSoyad ?? `${item.MusteriAdi ?? ""} ${item.MusteriSoyadi ?? ""}`).trim() || "-";
              const gender = (item.CinsiyetAdi ?? "").trim() || "-";
              const marital = (item.MedeniDurumAdi ?? "").trim() || "-";
              const description = (item.Hakkimda ?? "").trim() || "-";
              const imageUrl = (item.MusteriResimUrl ?? "").trim();
              const active = item.FavoriAktif !== false;
              const removing = removeLoadingId != null && musteriNr === removeLoadingId;
              const features = featureLines(item);

              return (
                <article
                  key={`${musteriNr ?? "seeker"}-${index}`}
                  className="overflow-hidden rounded-[26px] border border-[#e5e7eb] bg-white shadow-sm"
                >
                  <div className="flex items-start justify-between p-5">
                    <div>
                      <h2 className="text-lg font-black text-[#111827]">{fullName}</h2>
                      <p className="mt-1 text-sm text-[#6b7280]">{itemLocation(item) || "-"}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setConfirmRemoveItem(item)}
                      disabled={removing || musteriNr == null}
                  ...
                  <div className="px-5 pb-5">
                    <div className="mb-4 flex items-center gap-4">
                      {imageUrl ? (
                        ...
                        <img src={imageUrl} alt={fullName} className="h-16 w-16 rounded-2xl object-cover" />
                      ) : (
                      ...
                      {features.length ? (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {features.slice(0, 8).map((feature, featureIndex) => (
                            <span
                              key={`${musteriNr ?? index}-${featureIndex}`}
