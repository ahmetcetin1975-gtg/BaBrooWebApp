...
export async function POST(req: Request) {
  ...
  const pathBase = process.env.JOBSEEKERS_FAVORITE_ADD_PATH ?? "/api/musteri-fav-musteri/add-favorite";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to add favorite job seeker", ...data },
      { status: res.status }
    );
  }
