...
export async function POST(req: Request) {
  ...
  const favMusteriNr = parsePositiveInt(searchParams.get("favMusteriNr"));
  const dil = normalizeDil(searchParams.get("dil"));
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.JOBSEEKERS_FAVORITE_KAYNAK ?? "2");
  const h = await headers();
  const jar = await cookies();
  const hasBearerHeader = (h.get("authorization") ?? "").trim() !== "";
  const hasCookieToken = (jar.get("gtg_access")?.value ?? "").trim() !== "";

  if (favMusteriNr == null) {
    return NextResponse.json({ message: "Invalid favMusteriNr" }, { status: 400 });
  }
  if (!hasBearerHeader && !hasCookieToken) {
    return NextResponse.json({ message: "Bearer token is required" }, { status: 401 });
  }

  const pathBase = process.env.JOBSEEKERS_FAVORITE_REMOVE_PATH ?? "/api/musteri-fav-musteri/remove-favorite";
  const qp = new URLSearchParams({
    favMusteriNr: String(favMusteriNr),
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to remove favorite job seeker", ...data },
      { status: res.status }
    );
  }
