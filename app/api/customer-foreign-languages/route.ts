...
export async function GET(req: Request) {
  ...
  const pathBase =
    process.env.CUSTOMER_FOREIGN_LANGUAGES_GET_ALL_PATH ?? "/api/MusteriYdiller/getall";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load foreign languages", ...data },
      { status: res.status }
    );
  }
...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_FOREIGN_LANGUAGES_SAVE_KAYNAK ?? "2"
  );
  ...
  const pathBase =
    process.env.CUSTOMER_FOREIGN_LANGUAGES_SAVE_PATH ?? "/api/MusteriYdiller/save";
  const path = `${pathBase}?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "POST", body });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save foreign language", ...data },
      { status: res.status }
    );
  }
