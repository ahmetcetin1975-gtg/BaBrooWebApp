...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.JOBS_VIEW_KAYNAK ?? "2");
  ...
  const pathBase = process.env.JOBS_VIEW_ADD_BAK_PATH ?? "/api/musteri-bak-ilan/add-bak";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to update job view", ...data },
      { status: res.status }
    );
  }
