...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.PRODUCTS_IMAGES_SAVE_ALL_KAYNAK ?? "2"
  );
  ...
  if (alreadyUsesNewShape) {
    ...
    for (const [key, value] of entries) {
      if (key === "AddFiles" || key === "IsDefault" || key === "FirstUrunResimNr") {
        continue;
      }
  ...
  const pathBase = process.env.PRODUCTS_IMAGES_SAVE_ALL_PATH ?? "/api/urun-resim/save-all-web";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to save product images", ...data },
      { status: res.status }
    );
  }
