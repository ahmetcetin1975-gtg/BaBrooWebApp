...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.PRODUCTS_SAVE_KAYNAK ?? "2");
  ...
  const pathBase = process.env.PRODUCTS_SAVE_PATH ?? "/api/Urun/save";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save product", ...data },
      { status: res.status }
    );
  }
