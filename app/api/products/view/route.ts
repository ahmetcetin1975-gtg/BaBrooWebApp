...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.PRODUCTS_VIEW_KAYNAK ?? "1");
  ...
  const pathBase =
    process.env.PRODUCTS_VIEW_ADD_OR_UPDATE_PATH ?? "/api/musteri-bak-urun/add-or-update";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to update product view", ...data },
      { status: res.status }
    );
  }
