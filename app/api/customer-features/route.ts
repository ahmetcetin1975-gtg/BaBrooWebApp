...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.CUSTOMER_FEATURES_GET_ALL_PATH ?? "/api/MusteriOzellikleri/getall";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load customer features", ...data },
      { status: res.status }
    );
  }
...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_FEATURES_SAVE_KAYNAK ?? "2"
  );
  ...
  const pathBase = process.env.CUSTOMER_FEATURES_SAVE_PATH ?? "/api/MusteriOzellikleri/save";
  const path = `${pathBase}?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "POST", body });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save customer feature", ...data },
      { status: res.status }
    );
  }
