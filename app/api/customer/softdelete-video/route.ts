...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_SOFTDELETE_VIDEO_KAYNAK ?? "2"
  );
  ...
  const pathBase = process.env.CUSTOMER_SOFTDELETE_VIDEO_PATH ?? "/api/Musteri/softdelete-video";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to delete customer video", ...data },
      { status: res.status }
    );
  }
