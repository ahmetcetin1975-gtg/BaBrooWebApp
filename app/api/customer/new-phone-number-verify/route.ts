...
export async function POST(req: Request) {
  ...
  const otpCode = sanitizeDigits(body?.otpCode);
  ...
  const pathBase =
    process.env.CUSTOMER_NEW_PHONE_NUMBER_VERIFY_PATH ?? "/api/Musteri/new-phone-number-verify";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to verify new phone code", ...data },
      { status: res.status }
    );
  }
