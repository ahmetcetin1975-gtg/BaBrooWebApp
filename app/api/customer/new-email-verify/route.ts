...
export async function POST(req: Request) {
  ...
  const pathBase = process.env.CUSTOMER_NEW_EMAIL_VERIFY_PATH ?? "/api/Musteri/new-email-verify";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to verify new email code", ...data },
      { status: res.status }
    );
  }
