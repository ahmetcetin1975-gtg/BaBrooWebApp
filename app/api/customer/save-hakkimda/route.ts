...
export async function POST(req: Request) {
  ...
  const Hakkimda = typeof body?.Hakkimda === "string" ? body.Hakkimda.trim() : "";
  const pathBase = process.env.CUSTOMER_SAVE_HAKKIMDA_PATH ?? "/api/Musteri/save-hakkimda";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save about information", ...data },
      { status: res.status }
    );
  }
