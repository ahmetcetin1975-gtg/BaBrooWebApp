...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_SOFTDELETE_IMAGE_KAYNAK ?? "2"
  );
  const imageNoRaw = Number(searchParams.get("imageNo") ?? "1");
  const imageNo = imageNoRaw === 2 || imageNoRaw === 3 ? imageNoRaw : 1;
  ...
  const pathBase = process.env.CUSTOMER_SOFTDELETE_IMAGE_PATH ?? "/api/Musteri/softdelete-image";
  const qp = new URLSearchParams({
    dil: String(dil),
    kaynak: String(kaynak),
    imageNo: String(imageNo),
  });
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to delete customer image", ...data },
      { status: res.status }
    );
  }
