...
export async function POST(req: Request) {
  ...
  const pathBase =
    process.env.CUSTOMER_CONFIRM_PHONE_NUMBER_PATH ?? "/api/Musteri/confirm-phone-number";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to send phone verification code", ...data },
      { status: res.status }
    );
  }
