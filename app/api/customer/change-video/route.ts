...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_CHANGE_VIDEO_KAYNAK ?? "2"
  );
  ...
  const pathBase = process.env.CUSTOMER_CHANGE_VIDEO_PATH ?? "/api/Musteri/change-video";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to change customer video", ...data },
      { status: res.status }
    );
  }
