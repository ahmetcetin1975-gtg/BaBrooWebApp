...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.SERVICES_SAVE_KAYNAK ?? "1");
  ...
  const pathBase = process.env.SERVICES_SAVE_PATH ?? "/api/Hizmet/save";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save service", ...data },
      { status: res.status }
    );
  }
