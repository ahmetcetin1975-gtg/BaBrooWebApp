...
async function handleRemoveFavorite(req: Request) {
  ...
  const pathBase =
    process.env.SERVICES_FAVORITE_REMOVE_PATH ?? "/api/musteri-fav-hizmet/remove-favorite";
  ...
  return NextResponse.json(
    {
      message: lastResult?.data?.message ?? lastResult?.data?.Message ?? "Failed to remove favorite service",
      ...lastResult?.data,
    },
