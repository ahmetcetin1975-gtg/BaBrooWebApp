...
export async function GET(req: Request) {
  ...
  const basePath = process.env.SERVICES_GET_BY_ID_PATH ?? "/api/Hizmet/get";
  const qp = new URLSearchParams({ dil: String(dil) });
  const path = `${basePath}/${hizmetNr}?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "GET" });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load service detail", ...data },
      { status: res.status }
    );
  }
