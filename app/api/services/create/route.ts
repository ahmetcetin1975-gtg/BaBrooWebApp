...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.SERVICES_CREATE_KAYNAK ?? "2");
  ...
  const pathBase = process.env.SERVICES_CREATE_PATH ?? "/api/Hizmet/create";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to create service", ...data },
      { status: res.status }
    );
  }
