...
export async function GET(req: Request) {
  ...
  const hizmetNr = parseRequiredPositiveInt(searchParams.get("hizmetNr"));
  ...
  const basePath = process.env.SERVICES_UNAPPROVED_IMAGES_BASE_PATH ?? "/api/Hizmet/hizmet";
  const qp = new URLSearchParams({ dil: String(dil) });
  const path = `${basePath}/${hizmetNr}/resimleronaysiz?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "GET" });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load unapproved service images", ...data },
      { status: res.status }
    );
  }
