...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.ILAN_CREATE_OZELLIK_KAYNAK ?? "2");
  ...
  const pathBase = process.env.ILAN_CREATE_OZELLIK_PATH ?? "/api/ilan/create-ozellik";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save job post feature", ...data },
      { status: res.status }
    );
  }
