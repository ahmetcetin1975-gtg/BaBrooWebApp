...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.ILAN_SOFTDELETE_OZELLIK_KAYNAK ?? "2");
  ...
  const pathBase = process.env.ILAN_SOFTDELETE_OZELLIK_PATH ?? "/api/ilan/softdelete-ozellik";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to remove job post feature", ...data },
      { status: res.status }
    );
  }
