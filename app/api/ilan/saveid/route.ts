...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.ILAN_SAVEID_KAYNAK ?? "2");
  ...
  const pathBase = process.env.ILAN_SAVEID_PATH ?? "/api/ilan/saveid";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to update job post", ...data },
      { status: res.status }
    );
  }
