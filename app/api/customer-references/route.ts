...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.CUSTOMER_REFERENCES_GET_ALL_PATH ?? "/api/MusteriReferanslar/getall";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load references", ...data },
      { status: res.status }
    );
  }
...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_REFERENCES_SAVE_KAYNAK ?? "2"
  );
  ...
  const pathBase = process.env.CUSTOMER_REFERENCES_SAVE_PATH ?? "/api/MusteriReferanslar/save";
  const path = `${pathBase}?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "POST", body });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save reference", ...data },
      { status: res.status }
    );
  }
