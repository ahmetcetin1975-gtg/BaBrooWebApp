...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.SERVICE_OFFER_FEE_PATH ?? "/api/mesaj/hizmete-teklif-ver-coin";
  const path = `${pathBase}?${new URLSearchParams({ dil: String(dil) }).toString()}`;
  const { res, data } = await proxyJson({ path, method: "GET" });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to load service offer fee", ...data },
      { status: res.status }
    );
  }
