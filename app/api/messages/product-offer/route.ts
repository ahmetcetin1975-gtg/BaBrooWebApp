...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.PRODUCT_OFFER_KAYNAK ?? "2");
  ...
  const mesajUrunNr = toPositiveInt(body?.mesajUrunNr);
  const mesajMetin = sanitizeText(body?.mesajMetin);

  if (mesajUrunNr == null || !mesajMetin.trim()) {
    return NextResponse.json({ message: "mesajUrunNr and mesajMetin are required" }, { status: 400 });
  }

  const pathBase = process.env.PRODUCT_OFFER_PATH ?? "/api/mesaj/UruneTeklifVer";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to send product offer", ...data },
      { status: res.status }
    );
  }
