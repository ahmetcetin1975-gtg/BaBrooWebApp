...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.CUSTOMER_MESSAGE_FEE_PATH ?? "/api/mesaj/musteriye-mesaj-gonder-coin";
  const path = `${pathBase}?${new URLSearchParams({ dil: String(dil) }).toString()}`;
  const { res, data } = await proxyJson({ path, method: "GET" });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to load customer message fee", ...data },
      { status: res.status }
    );
  }
