...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.JOB_APPLICATION_FEE_PATH ?? "/api/mesaj/ilana-teklif-ver-coin";
  const path = `${pathBase}?${new URLSearchParams({ dil: String(dil) }).toString()}`;
  const { res, data } = await proxyJson({ path, method: "GET" });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to load job application fee", ...data },
      { status: res.status }
    );
  }
