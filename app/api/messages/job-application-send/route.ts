...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(searchParams.get("kaynak") ?? process.env.JOB_APPLICATION_SEND_KAYNAK ?? "2");
  ...
  const ilanNr = toPositiveInt(body?.ilanNr);
  const mesajMetin = sanitizeText(body?.mesajMetin);

  if (ilanNr == null || !mesajMetin) {
    return NextResponse.json({ message: "ilanNr and mesajMetin are required" }, { status: 400 });
  }

  const pathBase = process.env.JOB_APPLICATION_SEND_PATH ?? "/api/mesaj/IlanaBasvur";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? data?.Message ?? "Failed to send job application", ...data },
      { status: res.status }
    );
  }
