...
export async function GET(req: Request) {
  ...
  const musteriId = parseRequiredPositiveInt(searchParams.get("musteriid"));
  ...
  if (musteriId == null) {
    return NextResponse.json({ message: "Invalid musteriid" }, { status: 400 });
  }
  if (!hasBearerHeader && !hasCookieToken) {
    return NextResponse.json({ message: "Bearer token is required" }, { status: 401 });
  }

  const pathBase = process.env.MESSAGES_GET_BY_CUSTOMER_PATH ?? "/api/mesaj/getall-musteri-id";
  const qp = new URLSearchParams({
    dil: String(dil),
    search,
    page: String(page),
    pageSize: String(pageSize),
    musteriid: String(musteriId),
  });
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load customer messages", ...data },
      { status: res.status }
    );
  }
