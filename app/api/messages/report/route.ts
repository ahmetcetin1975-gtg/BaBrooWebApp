...
export async function POST(req: Request) {
  ...
  const pathBase = process.env.MESSAGES_REPORT_PATH ?? "/api/mesaj/raporet";
  ...
  if (!fallback.res.ok) {
    return NextResponse.json(
      { message: fallback.data?.message ?? primary.data?.message ?? "Failed to report message", ...fallback.data },
