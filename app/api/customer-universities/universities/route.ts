...
export async function GET(req: Request) {
  ...
  const ulkeId = (searchParams.get("ulkeId") ?? "").trim();
  ...
  const pathBase =
    process.env.CUSTOMER_UNIVERSITY_OPTIONS_PATH ?? "/api/Main/GetUniversiteler";
  const qp = new URLSearchParams({ dil: String(dil) });
  if (ulkeId) qp.set("ulkeId", ulkeId);
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load university options", ...data },
      { status: res.status }
    );
  }
