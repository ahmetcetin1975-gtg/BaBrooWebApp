...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.CUSTOMER_HIGH_SCHOOLS_GET_ALL_PATH ?? "/api/MusteriLiseler/getall";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load high schools", ...data },
      { status: res.status }
    );
  }
...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_HIGH_SCHOOLS_SAVE_KAYNAK ?? "2"
  );
  ...
  const pathBase = process.env.CUSTOMER_HIGH_SCHOOLS_SAVE_PATH ?? "/api/MusteriLiseler/save";
  const path = `${pathBase}?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "POST", body });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save high school", ...data },
      { status: res.status }
    );
  }
