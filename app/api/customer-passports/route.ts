...
export async function GET(req: Request) {
  ...
  const pathBase = process.env.CUSTOMER_PASSPORTS_GET_ALL_PATH ?? "/api/MusteriPasaportlar/getall";
  ...
  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to load passports", ...data },
      { status: res.status }
    );
  }
...
export async function POST(req: Request) {
  ...
  const kaynak = normalizeKaynak(
    searchParams.get("kaynak") ?? process.env.CUSTOMER_PASSPORTS_SAVE_KAYNAK ?? "2"
  );
  ...
  const pathBase = process.env.CUSTOMER_PASSPORTS_SAVE_PATH ?? "/api/MusteriPasaportlar/save";
  const path = `${pathBase}?${qp.toString()}`;
  const { res, data } = await proxyJson({ path, method: "POST", body });

  if (!res.ok) {
    return NextResponse.json(
      { message: data?.message ?? "Failed to save passport", ...data },
      { status: res.status }
    );
  }
